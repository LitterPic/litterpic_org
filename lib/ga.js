...
/** Fired when a user clicks a notification to navigate to a post. */
export function trackNotificationClicked(notificationType = '') {
    trackEvent('notification_clicked', { notification_type: notificationType });
}

/** Fired when a user likes a comment. */
export function trackCommentLiked(postId, commentId) {
    trackEvent('comment_liked', { post_id: postId, comment_id: commentId });
}

/** Fired when a user visits the donate page. */
export function trackDonatePageViewed() {
    trackEvent('donate_page_viewed');
}

/** Fired when a user generates a litter stats report. */
export function trackReportGenerated(filters = {}) {
    trackEvent('report_generated', filters);
}
