...
export function useAuth() {
    ...
    useEffect(() => {
		const unsubscribe = onAuthStateChanged(auth, (user) => {
			// Treat unverified users as unauthenticated in the app UI.
			// Do NOT call signOut() here — layout.js handles that so we don't
			// race against the SignUpForm writing the new user doc to Firestore.
