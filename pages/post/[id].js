...
export default function PostShareRedirect({ id, description, authorName, photoUrls, baseUrl }) {
    const router = useRouter();
    const [redirecting, setRedirecting] = useState(false);
    
    useEffect(() => {
        // Facebook's crawler executes some JS, but we want it to parse the meta tags first.
        // Waiting slightly ensures the tags are read before the route changes.
        const timer = setTimeout(() => {
            setRedirecting(true);
            router.push(`/stories?postId=${id}`);
        }, 1500); // 1.5 seconds gives crawlers plenty of time
    ...
    return (
        <div>
            <Head>
                <title>LitterPic Cleanup by {authorName}</title>
                <meta name="description" content={description} />
                
                {/* Open Graph / Facebook */}
                <meta property="og:type" content="article" />
                <meta property="og:url" content={`${baseUrl}/post/${id}`} />
                <meta property="og:title" content={`LitterPic Cleanup by ${authorName}`} />
                <meta property="og:description" content={description} />
                <meta property="og:site_name" content="LitterPic" />

                {/* Facebook uses the first og:image tag */}
                {photoUrls.map((url, index) => (
                    <React.Fragment key={`og-img-group-${index}`}>
                        <meta property="og:image" content={url} />
                        <meta property="og:image:secure_url" content={url.replace('http://', 'https://')} />
                    </React.Fragment>
                ))}
                ...
                <meta name="twitter:title" content={`LitterPic Cleanup by ${authorName}`} />
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', flexDirection: 'column', padding: '20px' }}>
                ...
                <h1 style={{ marginTop: '20px', fontFamily: 'sans-serif', textAlign: 'center' }}>LitterPic Cleanup by {authorName}</h1>
